...
function initializeFirebase() {
    ...
    loadFirebaseSDK()
        ...
        .catch((error) => {
            ...
            document.getElementById('modeDisplay').textContent = '⚠️ Offline Mode - Local Storage'; // Use modeDisplay for robust status
        });
}
...
function loadFromFirebase() {
    try {
        ...
        document.getElementById('modeDisplay').textContent = '🔄 Connecting to Cloud...'; // Use modeDisplay for robust status
        ...
        Promise.race([appDataRef.once('value'), timeoutPromise])
            .then((snapshot) => {
                if (snapshot.exists()) {
                    ...
                    document.getElementById('modeDisplay').textContent = '✅ Cloud Connected'; // Use modeDisplay for robust status
                    ...
                    document.getElementById('modeDisplay').textContent = '✅ Cloud Connected (No Data)'; // Use modeDisplay for robust status
                    loadFromStorage();
                    initializeUI();
                }
            })
            .catch((error) => {
                ...
                document.getElementById('modeDisplay').textContent = '⚠️ Offline Mode - Load Error'; // Use modeDisplay for robust status
                loadFromStorage();
                initializeUI();
            });
    } catch (error) {
        ...
        document.getElementById('modeDisplay').textContent = '⚠️ Offline Mode - Firebase Error'; // Use modeDisplay for robust status
        loadFromStorage();
        initializeUI();
    }
}
